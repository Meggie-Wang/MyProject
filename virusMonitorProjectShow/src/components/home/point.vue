...
<template lang="html">
  <div class="point">
    <v-header :name="$t('messages.dashboard.IPFingerDistribut')" :legendArr="$t('messages.dashboard.pointLegend')" :myChart="myChart"></v-header>
    <div class="main"></div>
  </div>
</template>
  ...
  watch: {
    styleActive(val) {
      this.myChart.dispatchAction({
        ...
        name: this.$t('messages.dashboard.pointLegend[1].name')
      })
    },
    language (val) {
      this.loadData()
    ...
    loadData() {
      api.point().then((res) => {
        ...
        that.options = {
          ...
          series: [
            {
              name: that.$t('messages.dashboard.pointLegend[0].name'),
              type: 'map',
              mapType: 'world',
              itemStyle: {
                normal: {
              ...
              data: that.countryDate
            },
            {
              name: that.$t('messages.dashboard.pointLegend[1].name'),
              type: 'effectScatter',
              coordinateSystem: 'geo',
              data: that.convertData(that.countryDate.sort(function (a, b) {
        ...
        let legendArr = that.options.series
