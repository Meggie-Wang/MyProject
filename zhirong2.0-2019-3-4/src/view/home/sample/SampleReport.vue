    ...
    <div class="SampleReport-container">
      ...
      <el-collapse v-model="activeNames">
        <el-collapse-item title="概述" name="1">
          ...
          <div v-if="relate_sonfile.length">
            <h1>子文件</h1>
            <div class="tableList">
              <table>
                <tr>
                ...
                <tr
                  v-if="i.id"
                  v-for="(i, j) in relate_sonfile"
                  :key="j">
                    <td>{{i.file_name}}</td>
                    <td>{{i.file_md5}}</td>
                    <td>{{i.is_malicious}}</td>
                    <td>
                      <el-tag
                        v-if="i.sample_label.length"
                        v-for="(sl, sl_index) in i.sample_label.split(',')"
                        :key="sl_index"
                        size="mini">{{sl}}</el-tag>
                      <span v-else>无</span>
                    </td>
                    <td>{{i.threaten_type}}</td>
                    <td align="center">
                      <img
                        class="__handCursor"
                        height="13"
                        :src="$img.view"
                        @click="checkSonFile(i)">
                    </td>
                  </tr>
              </table>
            </div>
...
export default {
  ...
  watch: {
    sampleReport (val) {
      ...
      if (val) {
        ...
        // 压缩文件的字文件列表
        this.relate_sonfile = val.basic_info.relate_sonfile.map(i => {
          // 恶意性
          i.is_malicious = (() => {
            let text = ''
            switch (i.is_malicious) {
              case 'yes':
                text = '安全'
                break
              case 'no':
          ...
          // 威胁类型
          i.threaten_type = i.threaten_type ? i.threaten_type : '无'
        ...
        if (Array.isArray(val.static_info.predict_result)) {
          if (val.static_info.predict_result.length) {
            console.log(val.static_info.predict_result)
  ...
  methods: {
    ...
    checkSonFile (r) {
      window.open(window.location.protocol + '//' + window.location.hostname + (window.location.port ? ':' + window.location.port : '') + '/#/SampleReport?md5=' + r.file_md5 + '&id=' + r.id + '&importLevel=' + r.important_level)
    },
    drawEcharts (data) {
      this.myChart = {}
      this.myChart = this.$echarts.init(document.querySelector('.behaviorLabel'))
