<template>
  <div class="wordList"
    ...
    <div class="main wordlist-inner"></div>
  ...
  watch: {
    ...
    myChartData (val) {
      if (val.length) {
        ...
        document.querySelector('.wordlist-inner').className = 'main wordlist-inner'
      }
    }
  },
  mounted () {
    ...
    if (Object.keys(this.myChart).length) {
      document.querySelector('.wordlist-inner').className += ' __hide'
    }
  },
  methods: {
    drawEcharts (data) {
