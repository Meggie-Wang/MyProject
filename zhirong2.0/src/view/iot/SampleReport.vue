<template>
  <div class="SampleReport" id="pdfDom">
    <div class="SampleReport-container">
      <div class="downPdf">
        <button
          v-if="(Object.getOwnPropertyNames(detect_result).length > 1 || Object.getOwnPropertyNames(basic_info).length > 1) && $tokenName !== 'gtd'"
      ...
      <el-collapse v-model="activeNames">
        <el-collapse-item title="概述" name="1">
          ...
          <div v-if="Object.getOwnPropertyNames(detect_result).length > 1">
            <h1>检测结果</h1>
            <div class="tableList">
              <table>
                <tr>
          ...
          <div v-if="Object.getOwnPropertyNames(basic_info).length > 1">
            <h1>基本信息</h1>
            <div class="tableList">
              <table>
                <tr>
                  <td width="200">威胁名称</td>
                  <td>{{ detect_result.threat_name || '无' }}</td>
                </tr>
                <tr>
          ...
          <div v-if="behaviorLabelShow">
            <!-- isNormalData(basic_info, basic_info.behavior_label, 'dada') -->
            <h1>行为标签</h1>
...
export default {
      ...
      if (val) {
        ...
        for (var i in val) {
          ...
          if (this.$tokenName !== 'gtd') {
            ...
            let haha = this.$common.splitObjToArr(pr, 6)
            this.predict_result = haha
          }
        }
        ...
        // console.log(this.isNormalData(this.basic_info, this.basic_info.behavior_label, 'daaa'))
        let d = document.querySelector('canvas')
        if (this.isNormalData(this.basic_info, this.basic_info.behavior_label, 'daaaa')) {
          this.behaviorLabelShow = true
          this.basic_info.behavior_label.forEach((i) => {
            this.legendName.push(i.name)
          })
          // console.log(222)
          if (d) { d.parentNode.style.height = '250px' }
          setTimeout(() => {
            this.drawEcharts(this.basic_info.behavior_label)
          }, 100)
        } else {
          // console.log(333)
          if (d) { d.parentNode.style.height = 0 }
          // .style.display = 'none'
  ...
  methods: {
    checkOriImg () {
      ...
      newwin.document.write('<img src=' + this.showGeneImg + ' />')
    },
    drawEcharts (data) {
      this.myChart = {}
    ...
    isNormalData (obj, arr, haha) {
      // if (haha) {
      //   console.log(haha)
      //   console.log(Object.getOwnPropertyNames(obj).length)
      //   console.log(arr)
      //   let h = Boolean(Object.getOwnPropertyNames(obj).length > 1 && arr && arr.length)
      //   console.log(h)
